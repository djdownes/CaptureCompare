...
directories="mESC_rep1,mESC_rep2,mESC_rep3,E10.5_rep1,E10.5_rep2,E10.5_rep3"           # Must be in same order as samples!
...
parameters="/t1-data/user/hugheslab/ddownes/Collaborations/Bee_Primative/beta_rerun/geneCapture/normalisation/all_plotting.txt"
...
normaliser="/t1-data/user/hugheslab/ddownes/Scripts/gitHubScripts/cis2way/cis2way_nomaliser.pl"

union="/t1-data/user/hugheslab/ddownes/Scripts/gitHubScripts/cis2way/cis2way_unionbdg.pl"

windower="/t1-data/user/hugheslab/ddownes/Scripts/gitHubScripts/cis2way/cis2way_windower.pl"

plotScripter="/t1-data/user/hugheslab/ddownes/Scripts/gitHubScripts/cis2way/cis2way_ggScripter.pl"
...
genomesSizes="/t1-data/user/hugheslab/ddownes/Scripts/gitHubScripts/cis2way_3way_files/${genome}.chrom.sizes"
...
for file in *.unionbdg
    ...
    awk '{printf "%s\t%d\t%d\t%.4f\t%.4f\n", $1,$2,$3,(($4+$5+$6)/3),(($7+$8+$9)/3)}' $file > "${oligoname}_TEMP_means.unionbdg"
...
#########################################
    ...
    echo "Preparing binned and windowed csv files for use in R"
    echo "Run command: perl ${windower} -viewpoints ${parameters} -samples ${sample1},${sample2}"
    echo

    perl ${windower} -viewpoints ${parameters} -samples ${sample1},${sample2}
